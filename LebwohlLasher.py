...
def timeInfo(nsteps,Ts,runtime,nmax):
  ...
  Arguments:
    nsteps (int) = number of Monte Carlo steps (MCS) performed;
    Ts (float) = reduced temperature (range 0 to 2)
    runtime (float) = time taken for the program to run
    nmax (int) = side length of square lattice to simulated.
  Description:
    Outputs to a csv file the important details to be plotted or presented later within the report.
    Importantly the lattice size, the amount of steps the termperatur and then the run time.
  """
