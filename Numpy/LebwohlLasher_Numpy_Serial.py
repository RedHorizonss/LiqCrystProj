...
def timeInfo(nsteps,Ts,runtime,nmax, plateu):
    filename = "Numpy/timeInfo_Serial.csv"
...
def MC_step(arr, Ts, nmax):
    ...
    for ix in range(nmax):
        for iy in range(nmax):
            ang = np.random.normal(scale=scale)  # Generate a random angle for this site
            ...
            if en1 <= en0 or np.exp(-(en1 - en0) / Ts) >= np.random.uniform(0.0, 1.0):
                # If the new configuration has lower energy or passes the Monte Carlo test, accept the change
                accept += 1
